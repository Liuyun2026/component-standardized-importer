...
**重要处理原则：**
- 执行过程中不需要中途停下来问用户问题。所有需要用户填写的内容（货源 URL、3D 模型调整表），**全部在最后一次性提出**。
- ⚠️ **绝不修改源文件！** 所有操作只读源目录、写入目标库目录。修改源文件是重大错误。

## 实现脚本

本 skill 提供了可直接执行的 Python 实现脚本：

- `scripts/kicad_import.py` — 完整的一站式导入脚本，使用 **S-Expression 解析器**（基于括号计数，跳过引号内内容）
...
- `*.kicad_mod` → 封装文件
- `*.kicad_sym` → 符号文件（提取每个 `(symbol "NAME"...` 块）
- `*.step` / `*.stp` → 3D 模型文件
...
- 字符串加引号：封装名、层名、焊盘编号
- Pad 格式：去掉 `solder_mask_margin`；层名加引号
...
**⚠️ 实现注意事项（踩坑记录）：**
1. **不要跳过所有 `)` 行！** 源封装中有多处单行 `)`（如 fp_text 的关闭括号），只有**最后一个非空行**的 `)` 才是封装根节点的关闭。正确做法：只替换最后一行的 `)`。
2. **fp_text 值的引号处理：** 源文件可能是 `REF**`（无引号）或 `"1"`（已有引号）。分别处理，避免重复加引号变成 `"""1"""`。
3. **fp_poly 的关闭括号：** 将 fp_poly 拆为 fp_line 后，原 fp_poly 末尾的 `)`（独立一行）需要跳过，否则会多一个游离括号。
4. **fp_text 多行结构：** `(effects ...)` 通常在下一行，其下还有一个 `)` 关闭。这些需要正确保留，不能误删。
...
2. **有则替换**（使用 S-Expression 解析器定位旧块范围并删除），**无则追加**
...
- 具体需对齐的点（**以目标库中已有符号的真实写法为准**）：
  - **Color**：arc/stroke 是否包含 `(color ...)` 字段——有则保留，无则去除
  - **hide 写法**：目标库可能用 `(hide yes)`、裸 `hide` 标志、或 `) hide)` 三种之一
  - **property 格式**：多行 id 格式或单行紧凑格式——对齐
  - **缩进和括号间距**：观察 `(effects (font (size s s) )` 里的空格习惯，保持一致
- 只改语法，不改引脚编号、位置、长度、图形

**⚠️ 实现注意事项：**
1. **必须使用 S-Expression 解析器**提取符号块（`extract_sexpr` 函数）。不要用 `re.DOTALL + .*?\)\s*\)` 这种非贪婪正则，符号体内嵌套括号多，非贪婪会提前截断。
2. **删除旧符号也要用 S-Expression 解析器**：`target.find('(symbol "NAME"')` 定位后，用 `extract_sexpr` 找到完整块范围删除。

### Step 6 — 符号合并后结构校验（关键！）

合并完成后，必须验证目标 `库名.kicad_sym` 的 **S-Expression 括号结构** 完整：
1. 确保 `(symbol ...)` 块全部位于 `(kicad_symbol_lib ...)` 内部
2. 检查开头和结尾，没有**多余括号**导致符号被甩到库外面
3. 统计 `(` 和 `)` 数量，必须相等
