...
- **目标库根路径**（如 `D:\...\TEST\lib\`）——结构须为：
  - `lib/lib.3dshapes/` ← 3D 模型存放
  - `lib/lib.pretty/` ← 封装存放
  - `lib/lib.kicad_sym` ← 符号合并到此文件
...
向用户询问该元件的商品页面 URL（如 DigiKey、LCSC、Mouser 等），然后：
1. 用 WebFetch 或 WebSearch 获取数据手册 URL、型号描述
2. 从 URL 或页面内容中提取货源编号（如 DigiKey 的 2445628、LCSC 的 C20617215）
3. 如果货源是 DigiKey，编号前加 `DJ` 前缀

### Step 4 — 转换封装文件

从目标库路径中提取**库名**：目标库根目录下的 `xxx.kicad_sym` 文件名即为库名（如 `test_skill.kicad_sym` → 库名为 `test_skill`）。
...
- 在文件末尾、`)` 结束之前，添加 3D 模型引用，路径格式为 `${库名}/库名.3dshapes/模型文件名.step`
  - 例如库名为 `test_skill`，则路径为 `${test_skill}/test_skill.3dshapes/xxx.step`
  - 参考目标库中已有封装（如 STM32）的 model 路径写法来确认格式
- 如果源封装已有 `(model ...)` 块，保留其 offset/scale/rotate 值不变

### Step 5 — 处理 3D 模型

将 `.step` 文件复制到 `lib/lib.3dshapes/` 目录。

生成一个 **3D 调整表格** 发给用户，包含 9 个参数：
...
表格中填写当前封装文件中该模型的原始值。用户如果修改后发回，按新值更新；如果没改发回或不回复，保持不动。
...
1. 读取目标 `lib.kicad_sym`，检查是否已有同名符号
2. **有则替换**，**无则追加**
3. 修改符号内的 `(property "Footprint" ...)` 为 `"lib:封装名"`
4. 补充/更新以下 property（参考库中其他元件的格式）：
   - `(property "Datasheet" ...)` — 填入 Step 3 获取的 URL
   - `(property "Description" ...)` — 填入型号描述
   - `(property "DK Part" ...)` 或 `(property "LCSC Part" ...)` — 按货源填写

**符号格式转换：**
- 字符串加引号
- 读取目标 `lib.kicad_sym` 中已存在的符号（如 STM32），分析其 property 格式、indent 风格、stroke 写法，使新插入的符号与之完全一致
...
合并完成后，必须验证目标 `lib.kicad_sym` 的 **S-Expression 括号结构** 完整：
...
在完成上述步骤后，向用户提交一个结构化的 3D 模型调整表，等待用户反馈。
